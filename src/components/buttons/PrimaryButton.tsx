...
const PrimaryButton = ({
  ...
  return (
    <button
      ...
      className={`${className} ${
        outline
          ? "text-blue-800 bg-white border hover:bg-[#0e75bc] hover:text-white"
          : "bg-[#0e75bc] text-white hover:bg-blue-dark "
      } rounded outline-none border-blue-800 `}
    >
      {loading ? (
        <div className="flex flex-row items-center justify-center font-semibold md:p-3 p-2 capitalize">
          <div
            className={`animate-spin rounded-full h-5 w-5 mr-2 border-t-2  border-b-2 ${
              outline
                ? "border-blue-800 hover:border-white"
                : "border-white"
            }`}
          ></div>
